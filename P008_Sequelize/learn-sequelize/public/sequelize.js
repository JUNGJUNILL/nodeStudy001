    ...
    if (xhr.status === 200) {
      ...
      comments.map(function (comment) {
        ...
        edit.addEventListener('click', function () { // 수정 클릭 시
          ...
          xhr.open('PATCH', '/comments/' + comment.id+'/seq/' + comment.seq);
        ...
        remove.addEventListener('click', function () { // 삭제 클릭 시
          ...
          xhr.open('DELETE', '/comments/' + comment.id+'/seq/' + comment.seq);
          xhr.send();
        });
//---------------------------------------- 수정 버튼
        td = document.createElement('td');
...
document.getElementById('user-form').addEventListener('submit', function (e) {
  e.preventDefault();
  var testid   =e.target.userids.value; 
  ...
  xhr.open('POST', '/users');
  xhr.setRequestHeader('Content-Type', 'application/json');
  xhr.send(JSON.stringify({id: testid, name: name, age: age, married: married }));
  e.target.userids.value = ''; 
